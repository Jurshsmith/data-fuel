...
impl<V> NodePointer<V> {
    ...
    pub fn set_prev(&self, prev: &Self) {
        unsafe { (*self.as_ptr()).prev = Some(prev.into()) }
    }
    fn remove_prev(&self) {
        unsafe { (*self.as_ptr()).prev = None }
    }
    fn get_next(&self) -> Option<&Self> {
        unsafe { (*self.as_ptr()).next.as_ref() }
    }
    fn get_prev(&self) -> Option<&Self> {
        unsafe { (*self.as_ptr()).prev.as_ref() }
    }
...
pub struct NodePointerIter<'a, V> {
    current: Option<&'a NodePointer<V>>,
}
...
impl<V> List<V> {
    ...
    pub fn pop(&mut self) -> Option<V> {
        if let Some(old_head) = &self.head {
            let new_head: Option<NodePointer<_>> = old_head.get_next().map(|new_head| {
                new_head.remove_prev();
                new_head.into()
            });
...
pub struct ListIter<'a, V> {
    next_front: Option<&'a NodePointer<V>>,
    next_back: Option<&'a NodePointer<V>>,
    len: usize,
}
...
impl<'a, V> Iterator for ListIter<'a, V> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.len == 0 {
            ...
            self.next_front.map(|node_pointer| {
                self.next_front = node_pointer.get_next();
                self.len -= 1;
...
impl<'a, V> DoubleEndedIterator for ListIter<'a, V> {
    fn next_back(&mut self) -> Option<Self::Item> {
        if self.len == 0 {
            None
        } else {
            self.next_back.map(|node_pointer| {
                self.next_back = node_pointer.get_prev();
                self.len -= 1;
                node_pointer.get_value()
            })
        }
